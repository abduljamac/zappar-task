...
table.addEventListener('click', (e) => {
    ...
    if (eventTarget && eventTarget.matches('.trash')) {
        const item = (_a = eventTarget.parentElement) === null || _a === void 0 ? void 0 : _a.parentElement;
        item.remove();
    }
});
document.querySelectorAll('.add-btn').forEach((btn) => btn.addEventListener('click', () => {
    let randomUser = users[Math.floor(Math.random() * Object.keys(users).length)];
